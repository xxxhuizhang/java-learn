...
import com.cbhlife.mybatis.dao.DepartmentMapper;
...
public class MyBatisCacheTest {
     ...
     * <p>
     * 和缓存有关的设置/属性：
     * 1）、cacheEnabled=true：false：关闭缓存（只关闭二级缓存,一级缓存一直可用)
     ...
     * 5）、localCacheScope：本地缓存作用域：(SESSION|STATEMENT)（一级缓存SESSION）；当前会话的所有数据保存在会话缓存中；
     * STATEMENT：可以禁用一级缓存；
     * <p>
     * 第三方缓存整合：
     * 1）、导入第三方缓存包即可；
    ...
    /**
     * DEBUG 08-23 22:59:13,379 Cache Hit Ratio [com.atguigu.mybatis.dao.EmployeeMapper]: 0.0  (LoggingCache.java:62)
     * DEBUG 08-23 22:59:13,401 ==>  Preparing: select * from tbl_employee where id = ?   (BaseJdbcLogger.java:159)
     * DEBUG 08-23 22:59:13,442 ==> Parameters: 1(Integer)  (BaseJdbcLogger.java:159)
     * DEBUG 08-23 22:59:13,474 <==      Total: 1  (BaseJdbcLogger.java:159)
     * Employee [id=1, lastName=Tom1, email=jerry@atguigu.com, gender=0]
     * DEBUG 08-23 22:59:13,497 Cache Hit Ratio [com.atguigu.mybatis.dao.EmployeeMapper]: 0.5  (LoggingCache.java:62)
     * Employee [id=1, lastName=Tom1, email=jerry@atguigu.com, gender=0]
     *
     * @throws IOException
     */
